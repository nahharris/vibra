...
Require-Content $linuxDockerfile 'ARG RUST_DEV_IMAGE=rust:1\.94\.1-alpine@sha256:77237dd363a0b127bb5ef532c2d64c0deb380b738e43a9c4bdac73398d6d0a08' 'Pinned clean Linux development base'
Require-Content $linuxDockerfile 'FROM \$\{RUST_DEV_IMAGE\} AS dev-builder' 'Isolated Linux development build stage'
Require-Content $linuxDockerfile 'FROM \$\{RUST_DEV_IMAGE\} AS dev' 'Clean Linux development final stage'
Require-Content $linuxDockerfile 'apk add --no-cache build-base clang-dev clang-static git llvm-dev llvm-static openssl-dev openssl-libs-static pkgconf zlib-static' 'Linux development toolchain packages'
Require-Content $linuxDockerfile 'LLVM_CONFIG_PATH=/usr/bin/llvm21-config' 'Alpine versioned LLVM discovery'
...
Require-Content $baseImages '^RUST_DEV_IMAGE=rust:1\.94\.1-alpine@sha256:77237dd363a0b127bb5ef532c2d64c0deb380b738e43a9c4bdac73398d6d0a08$' 'Scheduled rebuild Linux development base digest'
