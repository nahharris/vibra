...
> Historical record only. This document describes the removed YAML source surface and is not a current Vibra language contract. Current Vibra source uses S-expressions; see [`docs/decisions/s-expression-language.md`](../decisions/s-expression-language.md).
...
The **`vibra` CLI** in this repo implements a **reference subset** for bootstrapping (see [README.md](../../README.md)); the sections below preserve historical design material and are not authoritative.
