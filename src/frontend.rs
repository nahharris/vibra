    ...
    match form {
        ...
        TopLevel::Deffect(deffect) => {
            for operation in &mut deffect.operations {
                expand_exprs(
                    &mut operation.function.body,
                    module_path,
                    package_root,
                    embedded,
                )?;
                expand_annotations(
                    &mut operation.function.annotations,
                    module_path,
                    package_root,
                    embedded,
                )?;
...
fn visit_expr(expr: &Expr, visitor: &mut impl FnMut(&Expr) -> Result<()>) -> Result<()> {
    ...
    match &expr.value {
        ...
        ExprKind::Intrinsic { arguments, .. } => visit_exprs(arguments, visitor),
        ExprKind::Do(arguments) | ExprKind::Tuple(arguments) | ExprKind::Array(arguments) => {
    ...
    match form {
        ...
        TopLevel::Deffect(deffect) => {
            for operation in &deffect.operations {
                ...
                visit_type(&operation.function.return_type, &mut validate_name)?;
                for annotation in &operation.function.annotations {
                    if let AnnotationKind::Effects(row) = &annotation.value {
                        for label in &row.labels {
                            visit_type(label, &mut validate_name)?;
                        }
                ...
                for expr in &operation.function.body {
                    visit_expr_aliases(expr, &mut validate_name)?;
                }
            }
            Ok(())
