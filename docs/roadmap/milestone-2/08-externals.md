...
parsed `(import text @std.text)`, rejects every additional or differently
targeted import, and resolves the two qualified names directly through
capability-backed checked IR. It performs no source rewriting, ambient-prelude
lookup, or compatibility fallback. The two independent interpreter cases pass
with an empty audit trace; the complete corpus is 160 passed cases with zero
failures or unavailable observations.
