...
| C9 | 13 | Reserved project-wide `@tests` unit and optional confined `tests/` root; test-relative module identity, relationship to targets, import/visibility rules, exact canonical selector, deterministic discovery, module-local decoded-name uniqueness, and result contract. Define how pure assertions produce test failures without hidden host effects, general exceptions, generics, or fabricated `result` types. |
