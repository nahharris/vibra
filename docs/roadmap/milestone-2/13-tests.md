...
1. Capture the optional confined top-level `tests/` root as reserved unit
   `@tests`, with test-relative module identities in the immutable project
   graph. Reuse graph/resolver/checker services; do not add a test-only parser,
   hidden prelude, private-visibility escape, or executable test in a data file.
2. Check unique module-local decoded string names and empty effects for admitted
   tests. Apply normal public visibility to test imports of targets. Reject
   malformed/type-invalid tests before execution according to C9's scope.
...
4. Isolate values/initialization state and audit trace for each test. Use the same
   interpreter as `run`; use C10's canonical module-plus-string selector and
...
tests and independently authored `V1-PROJECT-*` / `V1-RUNTIME-*` cases through
the real `workspace-test` handler. Done includes graph and `@test-run.v1`
schema validation, process exits, empty per-test traces, and a clean initialized
