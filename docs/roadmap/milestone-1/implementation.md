...
| Declaration/type contextual structure | Step 8 | The internal source AST is an owned view over the lossless CST for recognized native declaration roots. It preserves raw body/pattern nodes, validates declaration/type context and fixed arities, maps errors to the closed registry, and adds no public JSON AST schema. Arbitrary nondeclaration reader fragments retain syntax-only acceptance. |
