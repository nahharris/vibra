...
Supersedes an earlier effect-system handoff, which was written against the YAML
surface and the `$policy`/`$capability` authority system decommissioned in #213.
That document's premise — that effects are expressed as capability
...
without its own sandboxing, and [the philosophy](philosophy.md) continues to
say so.
...
performs is allowed. Inference never fills the declaration in. Per the
philosophy page's *Explicit Intent*, inference may narrow a local expression but
